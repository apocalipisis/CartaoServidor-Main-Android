...
class ComerciantegerenteFuncionarioViewModel(val matricula: String, val token: String) :
    ViewModel() {
    ...
    fun consultarFuncionarios() {
        ...
        viewModelScope.launch {
            try {
                response =
                    APIComercianteGerente.APIComercianteGerenteService.nConsultarFuncionariosComercianteGerente(
                        matricula,
                        nConsulta,
                        token
                    )
...
class ComerciantegerenteFuncionarioViewModelFactory(
    private val matricula: String,
    private val token: String,
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(ComerciantegerenteFuncionarioViewModel::class.java)) {
            return ComerciantegerenteFuncionarioViewModel(matricula, token) as T
        }
